...
import { Agent, Signaler, createAgentContext, appStore, createTask } from './agent.js';
...
export function getToolDescriptionPrompt(role = 'agent') {
    ...
    const list = names.map(n => `- **${n}**: ${ALL_TOOL_DECLARATIONS[n]?.description || n}`).join('\n');
...
export class Toolbox {
    ...
    async execute_tool(name, args, context = {}) {
        ...
        if (context.updateTaskState) context.updateTaskState({ status: result.success ? 'tool_completed' : 'tool_failed' });
...
export const resolveSafePath = (base, inputPath, context = {}) => {
